...
const SalesReturnRecord = () => {
  ...
  useEffect(() => {
    ...
    if (savedReturns) {
      setSalesReturns(JSON.parse(savedReturns));
    }
  }, []);

  const handleDeleteReturn = (returnId: string) => {
    const updatedReturns = salesReturns.filter(returnItem => returnItem.id !== returnId);
    setSalesReturns(updatedReturns);
    localStorage.setItem('kairo_sales_returns', JSON.stringify(updatedReturns));

    toast({
      title: "تم حذف المرتجع",
      description: "تم حذف مرتجع المبيعات",
    });
  };
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>سجل مرتجع المبيعات</CardTitle>
          <CardDescription>
            عرض وإدارة مرتجعات المبيعات
          </CardDescription>
        </CardHeader>
        <CardContent>
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="max-w-2xl">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                عرض تفاصيل مرتجع المبيعات للعميل: {selectedReturn?.customerName}
                              </DialogDescription>
                            </DialogHeader>
                            {selectedReturn && (
...
export default SalesReturnRecord;
